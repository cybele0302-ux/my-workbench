...
const APP_VERSION = 'wobench-v29.63';
function fmtMoney(n) {
  var v = Number(n);
  if (!isFinite(v)) v = 0;
...
function fmtMoneyInt(n) {
  ...
  return v.toLocaleString('zh-CN', { minimumFractionDigits: 0, maximumFractionDigits: 0 });
    ...
    function renderHomeSummary() {
      ...
      let _guard = 0;
      while (true) {
        if (++_guard > 1500) break; // 防御：极端数据下避免死循环卡死主线程（v29.43）
        const ds = ymd(d);
    ...
    // v29.42 诊断：上次拉取的云端 payload 详情（记录数/大小/合并结果）
    let lastPullDiag = null;
    function renderCloudDiag() {
      ...
      // v29.42 诊断：显示上次拉取合并详情
      var diagLine = '';
      if (lastPullDiag) {
        diagLine = '<br>📊 上次拉取：云端' + lastPullDiag.cloudRecs + '条/' + (lastPullDiag.payloadSize / 1024).toFixed(1) + 'KB → '
          + '新增' + lastPullDiag.added + ' 更新' + lastPullDiag.updated + ' 独有' + (lastPullDiag.cloudExclusive - lastPullDiag.added)
          + ' 合并后本地' + lastPullDiag.finalLocal + '条';
      }
      el.innerHTML = '状态：' + (cloudCfg ? '已连接' : '未连接') + ' ｜ ' + sk +
        '<br>本地记录数：' + localN +
        '<br>上次上传：' + lp + ' ｜ 上次下载：' + lpl + errLine + diagLine;
    }
    ...
    // v29.43：剔除「无法被 JSON 序列化」的记录（循环引用/异常嵌套）。云端脏记录会导致整包序列化抛错、
    // 推送失败、且拉取后污染本地 → 连锁不同步。剔除后重新上推干净包，可自愈云端损坏数据。
    function stripUnserializable(obj) {
      if (!obj || !Array.isArray(obj.records)) return obj;
      var bad = [];
      var good = obj.records.filter(function (r) {
        try { JSON.stringify(r); return true; } catch (e) { bad.push((r && r.id) || '?'); return false; }
      });
      if (bad.length) console.warn('[v29.43] cloudEncrypt 剔除不可序列化的记录：', bad);
      return Object.assign({}, obj, { records: good });
    }
    async function cloudEncrypt(obj, pass) {
      ...
      try { plain = JSON.stringify(obj); }
        ...
        // 数据含循环引用/异常嵌套：剔除坏记录后重试，确保仍能上推（自愈云端）
        try { plain = JSON.stringify(stripUnserializable(obj)); console.warn('[v29.43] cloudEncrypt 已剔除坏记录后重加密：', se && se.message); }
        catch (se2) { throw new Error('序列化同步包失败（数据可能含循环引用或过大）：' + (se && se.message)); }
      }
    ...
    async function cloudPullFull(force) {
      ...
      try {
        ...
        saveLastCloudAt(lastKnownCloudAt);
        // v29.42 诊断：记录解密后的 payload 概况
        var localBefore = Object.keys(store).length;
        var cloudRecCount = Array.isArray(data.records) ? data.records.length : 0;
        var payloadSize = JSON.stringify(data).length;
        console.log('[v29.42 diag] 解密成功: 云端' + cloudRecCount + '条记录, payload=' + (payloadSize / 1024).toFixed(1) + 'KB, 本地当前' + localBefore + '条');
        try {
          var mergeResult = await mergeCloudData(data);
          // v29.42 诊断：保存合并结果到面板
          lastPullDiag = {
            cloudRecs: cloudRecCount,
            payloadSize: payloadSize,
            added: mergeResult.added || 0,
            updated: mergeResult.updated || 0,
            cloudExclusive: mergeResult.cloudExclusive || 0,
            finalLocal: Object.keys(store).length
          };
          console.log('[v29.42 diag] 合并结果:', JSON.stringify(lastPullDiag));
        } catch (me) {
    ...
    async function mergeCloudData(data) {
      ...
      // v29.42 返回合并统计供诊断面板展示
      var stat = { added: 0, updated: 0, cloudExclusive: 0 };
      ...
      Object.keys(store).forEach(function (id) {
        ...
        if (c) {
          ...
          if ((c.updatedAt || 0) > (local.updatedAt || 0)) { store[id] = c; stat.updated++; } // 云端更新，采用云端
      ...
      var resurrected = 0;
      stat.cloudExclusive = Object.keys(cloudById).length;
